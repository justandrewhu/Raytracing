...
SceneLoader::SceneData create_csg_scene() {
    ...
    // CSG: Sphere with holes (difference)
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (!scene_file.empty()) {
            ...
            std::cout << "Using built-in scene: " << builtin_scene << "\n";
            if (builtin_scene == "two_spheres") {
                ...
                std::cerr << "Unknown built-in scene: " << builtin_scene << "\n";
                return 1;
            }
        }
    ...
    auto framebuffer = renderer.render(scene_data.camera, scene_data.scene, width, height);
